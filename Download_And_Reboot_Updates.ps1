# Download available updates via Software Center and initiate a soft reboot
...
if ($SearchResult.Updates.Count -gt 0) {
    ...
    if ($DownloadResult.ResultCode -eq 2) { # 2 = wuDRCompleted
        Write-Output "Updates downloaded successfully. Initiating soft reboot."
        shutdown.exe /r /t 60 /c "System will restart in 60 seconds after updates download."
        exit 0
    } else {
